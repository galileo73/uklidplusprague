...
- Name: i18n.spec.ts >> i18n Language Switching >> should persist language in localStorage
- Location: tests\e2e\i18n.spec.ts:117:3
...
Locator: locator('header button').filter({ hasText: 'CZ' }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('header button').filter({ hasText: 'CZ' }).first()
